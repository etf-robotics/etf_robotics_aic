...
| [`joint_applied_torque`](../aic_task/tasks/manager_based/port_insertion/mdp/observations.py) | `mdp/observations.py` | `(N, len(joint_ids))` per-joint applied torque (N·m, signed) read from `data.applied_torque`. For `ImplicitActuator` this is IsaacLab's one-step-late PD recompute, since PhysX does not expose the exerted joint torque directly | `policy` (`joint_torque`) |
| [`body_incoming_wrench`](../aic_task/tasks/manager_based/port_insertion/mdp/observations.py) | `mdp/observations.py` | `(N, 6·k)` joint-reaction wrench `(Fx, Fy, Fz, Mx, My, Mz)` at each selected body, in that **body's own local frame** (matches `data.body_incoming_joint_wrench_b`'s convention — *not* the root frame the `*_b` pose/velocity helpers use; hence no `_b` suffix on this function) | `policy` (`wrist_wrench`, bound to the `wrist_ft` body role) |
