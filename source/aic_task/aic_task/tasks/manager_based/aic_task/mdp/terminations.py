...
class TerminationsCfg:
    ...
    success = DoneTerm(
        func=mdp.sfp_module_above_nic_card_success,
        params={
            "asset_cfg": SceneEntityCfg("robot", body_names=["sfp_module_visual"]),
...
class RewardsCfg:
    ...
    # -- Distance to NIC card --
    distance_to_nic_card = RewTerm(
        func=mdp.distance_to_nic_card_l2,
        weight=-3.0,
        params={
            "asset_cfg": SceneEntityCfg("robot", body_names=MISSING),
            "nic_card_cfg": SceneEntityCfg("nic_card"),
    ...
    ee_orientation_error_to_nic = RewTerm(
        func=mdp.ee_orientation_error_to_nic,
        weight=-2.1,
        params={
        "asset_cfg": SceneEntityCfg(
            name="robot",
            body_names=["gripper_tcp"],  # <- ime EE bodija na tvom robotu
        ),
        "nic_card_cfg": SceneEntityCfg(
            name="nic_card",
            # body_names=["port"],  # opcionalno ako imaš poseban body za port
        ),
        "z_rot_offset_deg": 180.0,  # eksplicitno, jasno
    },
    )
...
class AICTaskEnvCfg(ManagerBasedRLEnvCfg):
    ...
    def __post_init__(self) -> None:
        ...
        self.rewards.distance_to_nic_card.params["asset_cfg"].body_names = ee_body
