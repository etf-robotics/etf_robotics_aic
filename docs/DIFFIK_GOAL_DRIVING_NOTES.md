# DiffIK Goal-Driving Reminder

How to drive the robot toward the `insertion_goal` (or any EEF-frame pose) with
the relative-mode pose DiffIK action used by `AIC-Port-Insertion-v0`. Distilled
from the fix in [`scripts/random_agent.py`](../scripts/random_agent.py).
...
- `body_name = "gripper_tcp"` (the controlled TCP body, set via
  `ROBOT_ROLE_TCP` in
  [`source/aic_task/aic_task/asset_specs/robots.py`](../source/aic_task/aic_task/asset_specs/robots.py)
  and wired up in
  [`source/aic_task/aic_task/tasks/manager_based/port_insertion/builders.py`](../source/aic_task/aic_task/tasks/manager_based/port_insertion/builders.py))
...
**to the current TCP pose, expressed in the robot root (base) frame** — see
[`source/isaaclab/isaaclab/envs/mdp/actions/task_space_actions.py`](../../source/isaaclab/isaaclab/envs/mdp/actions/task_space_actions.py)
